...
endpoint_t mk_endpoint_proc(const char *name, Q_t q, char *argv[]) {
	...
	// if(pipe(sock) < 0) sys_die(pipe, "bad pipe");
	pid = fork();
	if (pid < 0) sys_die(fork, "bad fork");
	if (pid == 0) {
		if (close(sock[0]) < 0) sys_die(close, "bad close");
		if (dup2(sock[1], TRACE_FD_REPLAY) < 0) sys_die(dup2, "bad dup2");
		execvp(argv[0], argv);
	...
	if (close(sock[1]) < 0) sys_die(close, "bad close");
...
int has_data(endpoint_t *this, unsigned timeout_secs) {
	...
	if ((retval = select(this->fd + 1, &rfds, NULL, NULL, &tv)) < 0) {
		sys_die(select, "bad select");
	} else if(!retval) {
		printf("No data within %u seconds.\n", timeout_secs);
		return 0;
	} else {
		return 1;
	}
...
static int is_eof(endpoint_t *end) {
	if(!has_data(end, timeout_secs))
		err("process <%s> should have exited after 2 sec\n", end->name);

	char c;

	int nbytes = read(end->fd, &c, 1);

	if(nbytes < 0) sys_die(read, "bad read");
...
static int read_exact(endpoint_t *e, void *buf, int n, int can_fail_p) {
	...
	while(numRead < n) {
		if(!has_data(e, timeout_secs))
			return numRead;
		int bytesRead = read(e->fd, buf, n-numRead);
		// if(nbytes < 0 && !can_fail_p) sys_die(read, "bad read");
		if(bytesRead < 0 && !can_fail_p) sys_die(read, "bad read");
		if(bytesRead == 0) return numRead;
		numRead += bytesRead;
	}
...
void replay(endpoint_t *end, int corrupt_op) {
	...
	for(int n = 0; e; e = Q_next(e), n++) {
		...
		case OP_READ32:
		{
			unsigned u = e->val;
			if(n == corrupt_op) {
				if(signal(SIGPIPE, SIG_IGN) < 0) sys_die(signal, "can't catch signal");

				u = corrupt32(u);
				can_fail_p = 1;
			}
			
			write_exact(end, &u, 4, can_fail_p);
			break;
		}
		...
		case OP_WRITE32:
			...
			if(read_exact(end, &v, 4, can_fail_p) != 4) {
				if(can_fail_p) {
					...
					panic("read != 4 but should've been 4\n");
				}
			}
			break;
