...
#include "../bootloader/unix-side/tty.h"
...
static int do_builtin_cmd(int pi_fd, char *argv[], int nargs) {
  char buf[1024];
    ...
    if(nargs != 2){
      printf("Usage: get [core #]\n");
    }
    else{
      printf("Going to request output from pi\n");
      for(int i=0; i<nargs; i++){
        pi_put(pi_fd, argv[i]);
        pi_put(pi_fd, " ");
      }
    ...
    if(nargs != 2){
      printf("Usage: send [program]\n");
    }
    else{
      printf("Going to send program to pi\n");
      pi_put(pi_fd, "send\n");
      send_prog(pi_fd, argv[1]);
    }
    return 1;
  }
  return 0;
